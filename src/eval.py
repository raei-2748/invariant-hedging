...
_DEFAULT_RISK_KEYS: Tuple[str, ...] = ("ES95", "Mean", "SharpeRisk", "Turnover")
_MSI_EPS = 1e-8
...
def _evaluate_baselines(
    ...
    resolved_risk_keys: Tuple[str, ...] = tuple(risk_keys) if risk_keys else _DEFAULT_RISK_KEYS
    ...
    for name in names:
        ...
        for env_name, env in envs.items():
            ...
            summary = {
                ...
                **{risk: result["risks"].get(risk) for risk in resolved_risk_keys},
...
def _flatten_env_records(records: List[Dict], risk_keys: Sequence[str] | None = None) -> pd.DataFrame:
    resolved_risk_keys: Tuple[str, ...] = tuple(risk_keys) if risk_keys else _DEFAULT_RISK_KEYS
...
def _aggregate_records(
    ...
    for record in records:
        ...
        for env_name, metrics in env_metrics.items():
            ...
            env_entry = env_acc.setdefault(env_name, {risk: [] for risk in resolved_risk_keys})
            for risk in resolved_risk_keys:
    ...
    def _aggregate_gap(key: str) -> Dict[str, Dict[str, float]]:
        gap_acc: Dict[str, List[float]] = {risk: [] for risk in resolved_risk_keys}
...
def main(cfg: DictConfig) -> None:
    ...
    risk_keys: Tuple[str, ...] = tuple([*es_keys, "Mean", "SharpeRisk", "Turnover"])
    ...
    for split, split_envs in env_splits.items():
        for env_name, env in split_envs.items():
            ...
            risk_metrics = {
                k: float(v) for k, v in result["risks"].items() if v is not None and k in risk_keys
            }
    ...
    df = _flatten_env_records(evaluation_records, risk_keys=risk_keys)
        ...
        if baseline_names:
            ...
            for row in baseline_records:
                metrics = {
                    ...
                    for key in [*risk_keys, "mean_pnl", "sharpe"]
                    if key in row
                }
    ...
    aggregated = _aggregate_records(existing_records, risk_keys=risk_keys)
