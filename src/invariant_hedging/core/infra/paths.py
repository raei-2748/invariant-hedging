...
def canonical_run_dir(
    timestamp: str, experiment: str, root: str | Path = DEFAULT_ARTIFACT_ROOT
) -> Path:
    """Return the canonical run directory ``reports/artifacts/<timestamp>_<experiment>``."""
...
def episode_directory(run_dir: Path | str, *, seed: int, split: str, regime_name: str) -> Path:
    """Compute ``reports/artifacts/.../seeds/<seed>/<split>/<regime_name>``."""
