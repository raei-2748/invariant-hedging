...
def execution_cost(
    trade: torch.Tensor, spot: torch.Tensor, config: dict[str, float]
) -> torch.Tensor:
...
def apply_transaction_costs(
    positions: torch.Tensor, spot: torch.Tensor, config: dict[str, float]
) -> torch.Tensor:
    trades = torch.diff(
        torch.cat([torch.zeros_like(positions[..., :1]), positions], dim=-1), dim=-1
    )
