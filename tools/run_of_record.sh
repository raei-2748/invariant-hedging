...
usage() {
    cat <<'EOF'
Usage: tools/run_of_record.sh [options]
...
if [[ ${DRY_RUN} -eq 1 ]]; then
    ...
    echo "[DRY-RUN] checkpoint=\$(python3 tools/scripts/find_latest_checkpoint.py \"\$latest_run\")"
...
CHECKPOINT=$(python3 tools/scripts/find_latest_checkpoint.py "${LATEST_RUN}")
...
if [[ -n "${LATEST_EVAL:-}" ]]; then
    python3 tools/scripts/build_paper_diagnostics.py "${LATEST_EVAL%/}"
fi
